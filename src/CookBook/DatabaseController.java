...
public class DatabaseController implements Serializable {
	...
	public boolean insertUser(RegisteredUser registeredUser) {
		...
		try {
			...
			String strSelectUser = "select * from user where Username='" + registeredUser.getUserName() + "'";
			...
			if (!searchResult.next()) {
				String strInsertUser = "insert into user( Username , AccountID , Password)" + "values(?,?,?)";
				PreparedStatement sqlInsert = conn.prepareStatement(strInsertUser);
				sqlInsert.setString(1, registeredUser.getUserName());
				sqlInsert.setInt(2, registeredUser.getAccountID());
				sqlInsert.setString(3, registeredUser.getPassword());
	...
	public boolean insertComment(Comment comment) {
		...
		try {
			// We need to check whether the user name exists in the database
			String strSelectComment = "select * from comment where CommentID='" + comment.getCommentID() + "'";
			Statement sqlSearch = conn.createStatement();
			ResultSet searchResult = sqlSearch.executeQuery(strSelectComment);
			// Only when the user name does not exist in the database can the User be inserted
			if (!searchResult.next()) {
				String strInsertComment = "insert into comment( CommentID , RecipeID , AccountID , Context)" + "values(?,?,?,?)";
				PreparedStatement sqlInsert = conn.prepareStatement(strInsertComment);
				sqlInsert.setInt(1, comment.getCommentID());
				sqlInsert.setInt(2, comment.getRecipeID());
				sqlInsert.setInt(3, comment.getAccountID());
				sqlInsert.setString(4, comment.getContext());
				result = sqlInsert.executeUpdate();
				if (result == 1) {
					isSuccess = true;
				}
